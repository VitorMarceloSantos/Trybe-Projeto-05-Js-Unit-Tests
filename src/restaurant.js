...
let valorTotal = 0; // Variável com escopo Global
function atribuirValor(consumo, item) {
  for (let i = 0; i < consumo.length; i += 1) {
    if (item[0] === consumo[i]) {
      // console.log(`Ìtem: ${item[0]} - Consumo: ${consumo[i]}`);
       valorTotal += item[1];
    }
  }
}
...
// Função Pay
function functPay(objeto, consumo) {
  for (let key in objeto) {
    if (Object.prototype.hasOwnProperty.call(objeto, key)) {  
      Object.entries(objeto[key]).forEach((item) => { // Object.entries vai retornar um array com chave e valor: Exemplo - [ 'coxinha', 3.9 ]
        atribuirValor(consumo, item); 
      });
    }
  }

  valorTotal += (valorTotal * 0.1); // somando a taxa de 10%
  return valorTotal;
}
// console.log(functPay({ food: { 'coxinha': 3.9, 'sopa': 9.9, 'sashimi': 15.7}, drink: {'agua': 3.9, 'cerveja': 6.9}},['coxinha', 'agua', 'coxinha'])); // Testando a functPay

const createMenu = (objeto) => {
  let arrayOrder = [];
  const menu = {
    fetchMenu: () => objeto,
    consumption: arrayOrder,
    order: (produto) => (arrayOrder.push(produto)),
    pay: (objeto1, consumo) => functPay(objeto1, consumo),
    // pay: functPay(objeto, arrayOrder),
  };
  return menu;
};
