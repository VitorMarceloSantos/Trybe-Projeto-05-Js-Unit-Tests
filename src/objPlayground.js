...
function funcEntries(object) {
  let arrayEntries = [];
      for (let keys in object) {
        if (Object.prototype.hasOwnProperty.call(object, keys)) { // Utilizado para que o Eslinte não apresente erro na execução do for in (erro: guard-for-in)
          let arrayTemp = [keys, object[keys]];
          arrayEntries.push(arrayTemp);
        }
      }
    return arrayEntries;
}
